...
class legislacion extends contenidos{
	...
	*/
	function get_titulocontenido($pid){
		$rows = $this->db->get_results("SELECT cont_id as id_articulo,cont_titulo as Titulo,cont_contenido as Contenido FROM sys_temp_cont WHERE cont_cate_id={$pid} ORDER BY cont_id ASC");
...
// 	===========================================================================
	function get_codigopenalprocesal($pid){
		$rows = $this->db->get_results("SELECT cont_subtitulo as Titulo,cont_contenido as Contenido FROM sys_contenido WHERE cont_id={$pid}");
		foreach($rows as $cod=>$row){
			$row->Titulo = html_entity_decode($row->Titulo);
			$contenido  = $row->Contenido;
			$row->Contenido = html_entity_decode(strip_tags($row->Contenido));
...
// 	===========================================================================
	...
	function get_articulo($id,$tid){
		$rows = $this->db->get_results("SELECT cont_id as id_articulo,cont_titulo as Titulo,cont_contenido as Contenido,cont_cate_id FROM sys_temp_cont WHERE cont_tipo_id={$tid} AND (cont_titulo LIKE 'Articulo {$id}:%' OR cont_titulo LIKE 'Articulo {$id}.%' OR cont_titulo LIKE 'Articulo {$id} %') ORDER BY cont_id DESC");
		$pid = $rows[0]->cont_cate_id;
		$aid = $rows[0]->id_articulo;
		//print_r($rows);
		$rows = $this->db->get_results("SELECT cont_id as id_articulo,cont_titulo as Titulo,cont_contenido as Contenido FROM sys_temp_cont WHERE cont_cate_id={$pid} ORDER BY ({$aid}=cont_id) DESC,cont_id ASC");
		//die("SELECT ({$id}=cont_id),cont_id as id_articulo,cont_titulo as Titulo,cont_contenido as Contenido FROM sys_temp_cont WHERE cont_cate_id={$pid} ORDER BY ({$aid}=cont_id) DESC,cont_id ASC");
		foreach($rows as $cod=>$row){
			$row->Titulo = html_entity_decode($row->Titulo);
			$contenido  = $row->Contenido;
			$row->Contenido = html_entity_decode(strip_tags($row->Contenido));
	...
	//============================================000000
	function get_elperuano_fechas(){
		$result = $this->db->get_results("SELECT elpe_fecha as fecha FROM sys_elperuano");
        echo json_encode( $result );
	}
	//============================================000000

	function get_titulocontenido_jurisprudencia($pid){
